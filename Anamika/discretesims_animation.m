...
function  [gluc, mitopos, mitostate, opt] = discretesims_animation(options)
...
ind = 1;
vidfile = VideoWriter('mitomovie.mp4');
vidfile.FrameRate = 2;
...
%             %----------
    ...
    if (opt.dodisplay && mod(step,opt.showevery)==0)
        ...
        title(sprintf('Step %d: Accumulation Metric = %0.3f',step,mval))
        xlabel('L (dimensionless units)');
        ylabel('Glucose Profile (dimensionless concentration)')
        open(vidfile)
        if (mval>0.2 && ind<41)
            F(ind) = getframe(gcf);
