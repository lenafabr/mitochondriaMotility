%Constant permeability sims
%With Non-linear Michaelis-Menten Permeability
...
opt.cend = 0.1; 
%Permeability term
opt.P = 0.1; %permeability coefficient
opt.PKm = 3; %Or is it 30?
...
while (normdtg > dtcutoff)
    ...
    dtg(2:end-1) = Dh*d2g(2:end-1) - (kgh * opt.Km * opt.nmito * opt.msize) * (gluc(2:end-1) .* Tmito(2:end-1)) ./ (opt.Km + gluc(2:end-1)) + (Ph*PKmh*(C_out(2:end-1) - gluc(2:end-1))./ (PKmh + (C_out(2:end-1) - gluc(2:end-1))));
    normdtg = norm(dtg);
