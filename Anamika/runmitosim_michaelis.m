...
opt.D = 200;% glucose diffusion coefficient
opt.kw = 1; % rate of starting a walk
opt.ks = 1; % rate of stopping is ks*[gluc]
opt.kg = 0.2; % rate of glucose consumption (max rate is kg*Km)
opt.Km = 0.1/30; % limiting glucose concentration (relative to c0)
...
for step = 1:opt.nstep
    ...
    for mc = 1:opt.nmito
        ...
        eatrate = kgh*Kmh*gluc(ind1:ind2)/(Kmh + gluc(ind1:ind2));
        dtg(ind1:ind2) = dtg(ind1:ind2)-kgh*gluc(ind1:ind2);
    end
